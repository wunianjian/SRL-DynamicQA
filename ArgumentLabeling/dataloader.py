...
# +
# def label2query2(args_key_list, _id=None, arg_query_type=2, argm_query_type=1):
#     """
#     Args:
#         label1: ARG or ARGM
    ...
    def init_data(self, data, tokenizer, max_tokens, gold_level, arg_query_type, argm_query_type):
        for s_id, d in enumerate(tqdm(data, desc='preprocessing')):
            ...
            for i in range(len(predicates)):
                ...
                print(self.gold_senses)
                self.senses.append((s_id,pre,d['plemma_ids'][i].split('.')[0],d['plemma_ids'][i].split('.')[1]))
                print("gold_level = 0: ", lemmas[i]+'.'+frameset_ids[i])
                print("gold_level = 1: ", d['plemma_ids'][i])
                if gold_level == 0:
                ...
                print(fargs_key)
                print(argms_key)
#                 q_s = label2query2(fargs_key+argms_key, lem_id, arg_query_type, argm_query_type)
#                 q_s = [q.replace('X', pre_str) for q in q_s]
#                 for i, k in enumerate(fargs_key+argms_key):
#                     v = args1[k]
                for k in fargs_key+argms_key:
...
def load_data(path, pretrained_model_name_or_path, max_tokens, shuffle, dataset_tag, local_rank=-1, gold_level=0,
              arg_query_type=0, argm_query_type=0):
...
# data_loader, data_set = dev_dataloader = load_data(dev_path, pretrained_model_name_or_path, max_tokens, False,
...
# +
# data_set[0]["input_ids"].shape

# +
# tokenizer.decode(np.array(data_set[0]["input_ids"]))
# -
